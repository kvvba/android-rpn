...
class MyWidgetProvider : AppWidgetProvider(), Calculator {
    ...
    override fun onUpdate(
        ...
        appWidgetManager.getAppWidgetIds(getComponentName(context)).forEach {
            ...
            setupIntent(context, views, ENTER, R.id.btn_enter)
            ...
            setupIntent(context, views, SWAP, R.id.btn_swap)
            setupIntent(context, views, DROP, R.id.btn_drop)
            setupIntent(context, views, CHS, R.id.btn_chs)
            setupIntent(context, views, BACKSPACE, R.id.btn_backspace)
            setupIntent(context, views, RESET, R.id.btn_ac)
    ...
    private fun setupAppOpenIntent(context: Context, views: RemoteViews, id: Int) {
        val intent = Intent(context, com.jakub.rpncalculator.activities.MainActivity::class.java)
        val pendingIntent =
    ...
    private fun updateTextColors(views: RemoteViews, color: Int, context: Context) {
        val viewIds = intArrayOf(
            ...
            R.id.btn_power, R.id.btn_root, R.id.btn_swap, R.id.btn_drop, R.id.btn_chs,
            R.id.btn_backspace, R.id.btn_ac, R.id.btn_divide, R.id.btn_multiply, R.id.btn_minus,
            R.id.btn_plus, R.id.btn_decimal, R.id.btn_enter
        )
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (val action = intent.action) {
            DECIMAL, ZERO, ONE, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, ENTER, RESET,
            PLUS, MINUS, MULTIPLY, DIVIDE, PERCENT, POWER, ROOT, SWAP, DROP, CHS, BACKSPACE ->
    ...
    private fun myAction(action: String, context: Context) {
        ...
        when (action) {
            ...
            ENTER -> calc!!.handleEnter()
            SWAP -> calc!!.handleSwap()
            DROP -> calc!!.handleDrop()
            CHS -> calc!!.handleChs()
